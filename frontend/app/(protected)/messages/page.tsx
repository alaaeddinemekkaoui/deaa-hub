...
function GroupManageModal({
  ...
  const addMember = async (userId: number) => {
    ...
    try {
      ...
      if (newMember) {
        setMembers((prev) => [
          ...prev,
          { groupId: group.id, userId, canSend: false, user: newMember },
        ]);
      }
      setUserSearch("");
    } catch (err) {
  ...
  const toggleCanSend = async (userId: number, current: boolean) => {
    ...
    try {
      await api.patch(
        `/messaging/groups/${group.id}/members/${userId}/can-send`,
        { canSend: !current },
      );
      setMembers((prev) =>
        prev.map((m) =>
          m.userId === userId ? { ...m, canSend: !current } : m,
        ),
      );
    } catch (err) {
  ...
  const save = async () => {
    ...
    try {
      if (isCreate) {
        ...
        await api.patch(`/messaging/groups/${group!.id}`, {
          name: name.trim(),
        });
                        ...
                        >
                          ...
                          <label className="flex items-center gap-1 text-[10px] text-slate-500 cursor-pointer">
                            <input
                              ...
                              checked={m.canSend}
                              onChange={() =>
                                void toggleCanSend(m.userId, m.canSend)
                              }
                            />
                            Peut envoyer
                          </label>
...
function ComposeModal({
  ...
  const selectedRecipients = users.filter((u) =>
  ...
  const send = async () => {
    ...
    try {
      if (target === "group") {
        ...
        const uniqueIds = [
          ...new Set(recipientIds.map(Number).filter(Boolean)),
        ];
        await Promise.all(
            ...
            {target === "group" ? (
              ...
              <div className="space-y-1">
                  ...
                  <div className="mb-1 flex flex-wrap gap-1.5 rounded-lg border border-emerald-100 bg-emerald-50/60 p-2">
                    {selectedRecipients.map((u) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          className="rounded-full px-1 leading-none hover:bg-emerald-200"
                          onClick={() => removeRecipient(String(u.id))}
                        >
                          ×
                        </button>
                      </span>
                ...
                <select
                  className="input"
                  size={6}
                  value={highlightedRecipientId}
                  onChange={(e) => setHighlightedRecipientId(e.target.value)}
                  onDoubleClick={(e) =>
                    addRecipient((e.target as HTMLSelectElement).value)
                  }
                >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    onClick={() => addRecipient(highlightedRecipientId)}
                    disabled={!highlightedRecipientId}
                  >
                    Ajouter
                  </button>
                </div>
          ...
          <div className="flex gap-2 px-5 py-3 border-t border-slate-100">
            ...
            <button
              ...
              disabled={
                sending ||
                !content.trim() ||
                (target === "group" ? !groupId : recipientIds.length === 0)
              }
              onClick={() => void send()}
            >
              {sending ? (
...
export default function MessagesPage() {
  ...
  const [activeTab, setActiveTab] = useState<"groups" | "received" | "sent">(
    initialTab,
  );
  ...
  const [managingGroup, setManagingGroup] = useState<Group | null | "new">(
    null,
  );
  ...
  const sendableGroups = useMemo(() => {
    if (isAdmin) return compatibleGroups;
    return compatibleGroups.filter(
      (group) => group.isMember !== false && group.canSend,
    );
  }, [compatibleGroups, isAdmin]);

  const loadGroups = useCallback(async () => {
    try {
      const res = await api.get<Group[]>(
        `/messaging/groups?scope=${isAdmin ? "all" : groupScope}`,
      );
      setGroups(res.data);
    } catch {
      /* silent */
  ...
  useEffect(() => {
    setTabPages((prev) => ({ ...prev, groups: 1 }));
    if (activeThread?.kind === "group" && !isAdmin && groupScope === "mine") {
      const stillVisible = groups.some(
        (group) => group.id === activeThread.group.id,
      );
      if (!stillVisible) setActiveThread(null);
    }
  }, [activeThread, groupScope, groups, isAdmin]);
  ...
  useEffect(() => {
    ...
    if (peerIdParam) {
      ...
      const fromReceived = receivedThreads.find(
        (t) => t.peer.id === peerId,
      )?.peer;
      const fromSent = sentThreads.find((t) => t.peer.id === peerId)?.peer;
      const peer = fromReceived ??
        fromSent ?? {
          id: peerId,
          fullName: `Utilisateur #${peerId}`,
          role: "",
        };
    ...
    async (showLoader = false) => {
      ...
      try {
        ...
        if (activeThread.kind === "group") {
          res = await api.get<Message[]>(
            `/messaging/groups/${activeThread.group.id}/messages`,
          );
        } else {
          res = await api.get<Message[]>(
            `/messaging/conversation/${activeThread.peer.id}`,
          );
        }
        ...
        if (showLoader)
          toast.error(
            getApiErrorMessage(err, "Impossible de charger les messages."),
          );
      } finally {
  ...
  const filteredGroups = compatibleGroups.filter((g) =>
  ...
  const canSendInThread = (() => {
    ...
    return activeThread.group.canSend === true;
  })();
        ...
        >
          ...
          <div className="px-3 py-2 border-b border-slate-50">
            ...
            {!isAdmin && activeTab === "groups" && (
              <div className="mt-2 grid grid-cols-2 rounded-xl border border-slate-200 bg-slate-50 p-0.5">
                {(["mine", "all"] as const).map((scope) => (
                  <button
                    key={scope}
                    type="button"
                    className={cn(
                      "rounded-lg py-1 text-[11px] font-semibold transition-all",
                      groupScope === scope
                        ? "bg-white text-slate-800 shadow-sm"
                        : "text-slate-500",
                    )}
                    onClick={() => setGroupScope(scope)}
                  >
                    {scope === "mine" ? "Mes groupes" : "Tous"}
                  </button>
                ))}
              </div>
            )}
            <div className="mt-2 flex items-center justify-between gap-2 text-[11px]">
              ...
              <span className="text-slate-400">
                {activeCollectionLength} éléments
              </span>
            </div>
          </div>
              ...
              filteredGroups.length === 0 ? (
                ...
                <div className="py-1">
                  ...
                  {paginatedGroups.map((g) => {
                    const active =
                      activeThread?.kind === "group" &&
                      activeThread.group.id === g.id;
                    const canOpenGroup = isAdmin || g.isMember !== false;
                    return (
                      <div
                        key={g.id}
                        className={cn(
                          "group flex w-full items-center gap-3 px-4 py-2.5 transition-colors hover:bg-slate-50",
                          active && "bg-emerald-50",
                          !canOpenGroup && "opacity-70",
                        )}
                      >
                        <button
                          type="button"
                          ...
                          onClick={() => {
                            if (!canOpenGroup) {
                              toast.info(
                                "Groupe visible en mode Tous, mais non affecté à votre profil.",
                              );
                              return;
                            }
                            setActiveThread({ kind: "group", group: g });
                          }}
                        >
                          <div
                            className={cn(
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-400">
                              {g._count.members} membre
                              {g._count.members !== 1 ? "s" : ""} ·{" "}
                              {g._count.messages} msg
                              {g._count.messages !== 1 ? "s" : ""}
                              {!canOpenGroup ? " · Non affecté" : ""}
                            </p>
                          </div>
                        </button>
                        {isAdmin && (
                  ...
                  {paginatedReceived.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          setActiveThread({ kind: "direct", peer: item.peer })
                        }
                      >
              ...
              <div className="sticky bottom-0 border-t border-slate-100 bg-white px-3 py-2">
                <div className="flex items-center justify-between gap-2 text-[11px]">
                  ...
                  <span className="text-slate-500">
                    Page {tabPages[activeTab]} / {activePageCount}
                  </span>
                  <button
                    type="button"
                    className="rounded-md border border-slate-200 px-2 py-1 text-slate-600 disabled:opacity-50"
                    disabled={tabPages[activeTab] >= activePageCount}
                    onClick={() =>
                      setTabPages((prev) => ({
                        ...prev,
                        [activeTab]: Math.min(
        ...
        >
          {!activeThread ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 text-slate-400">
              ...
              <p className="text-sm">
                Sélectionnez un groupe ou composez un message
              </p>
              <button
                type="button"
              ...
              <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3">
                ...
                {activeThread.kind === "group" && (
                  <div
                    className={cn(
                      "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-white",
                      activeThread.group.type === "EVERYONE"
                        ? "bg-blue-500"
                        : activeThread.group.type === "ADMINS_ONLY"
                          ? "bg-red-500"
                          : activeThread.group.type === "CLASS"
                            ? "bg-amber-500"
                            : "bg-emerald-600",
                    )}
                  >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {activeThread.kind === "group" && (
                    <p className="text-[11px] text-slate-400">
                      {GROUP_TYPE_LABELS[activeThread.group.type]} ·{" "}
                      {activeThread.group._count.members} membres
                    </p>
                  )}
                  {activeThread.kind === "direct" && (
