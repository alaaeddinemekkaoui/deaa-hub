...
function getCurrentWeekDate(dayOfWeek: number) {
  ...
  currentDate.setUTCDate(
    currentDate.getUTCDate() + mondayOffset + (dayOfWeek - 1),
  );
...
async function main() {
  ...
  const classAPESA1 = await upsertClass(
    'APESA 1',
    1,
    filiereAPESA.id,
    cyclePrepa.id,
  ...
  const classAPESA2 = await upsertClass(
    'APESA 2',
    2,
    filiereAPESA.id,
    cyclePrepa.id,
  ...
  const classAgro1 = await upsertClass(
    'Agronomie 1A',
    1,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classAgro2 = await upsertClass(
    'Agronomie 2A',
    2,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classAgro3 = await upsertClass(
    'Agronomie 3A',
    3,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classPVPP4 = await upsertClass(
    'PVPP 4A',
    4,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classPVPP5 = await upsertClass(
    'PVPP 5A',
    5,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classPIA4 = await upsertClass(
    'PIA 4A',
    4,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classEDR4 = await upsertClass(
    'EDR 4A',
    4,
    filiereAgronomie.id,
    cycleIng.id,
  ...
  const classVeto1 = await upsertClass(
    'Vétérinaire 1A',
    1,
    filiereVeto.id,
    cycleVeto.id,
  ...
  const classVeto2 = await upsertClass(
    'Vétérinaire 2A',
    2,
    filiereVeto.id,
    cycleVeto.id,
  ...
  const classVeto3 = await upsertClass(
    'Vétérinaire 3A',
    3,
    filiereVeto.id,
    cycleVeto.id,
  ...
  const classVeto4 = await upsertClass(
    'Vétérinaire 4A',
    4,
    filiereVeto.id,
    cycleVeto.id,
  ...
  const classVeto5 = await upsertClass(
    'Vétérinaire 5A',
    5,
    filiereVeto.id,
    cycleVeto.id,
  ...
  const classGR1 = await upsertClass(
    'Génie Rural 1A',
    1,
    filiereGR.id,
    cycleIng.id,
  ...
  const classGR2 = await upsertClass(
    'Génie Rural 2A',
    2,
    filiereGR.id,
    cycleIng.id,
  ...
  const classIAA1 = await upsertClass(
    'IAA 1A',
    1,
    filiereIAA.id,
    cycleIng.id,
  ...
  const classSGIT1 = await upsertClass(
    'SGIT 1A',
    1,
    filiereSGIT.id,
    cycleIng.id,
  ...
  for (const s of laureateStudents) {
    ...
    await prisma.laureate.upsert({
      ...
      create: {
        studentId: saved.id,
        graduationYear: 2023,
        diplomaStatus: DiplomaStatus.retrieved,
      },
    });
  }
  ...
  // ─── Modules & Éléments ───────────────────────────────────────────────────
  // APESA modules
  const modMathApesa = await findOrCreateModule(
    'Mathématiques',
    'S1-S2',
    filiereAPESA.id,
    null,
  );
  const modPhyChimApesa = await findOrCreateModule(
    'Physique-Chimie',
    'S1-S2',
    filiereAPESA.id,
    null,
  );
  const modBioApesa = await findOrCreateModule(
    'Biologie et Géologie',
    'S1-S2',
    filiereAPESA.id,
    null,
  );
  const modInfoApesa = await findOrCreateModule(
    'Informatique et Numérique',
    'S1',
    filiereAPESA.id,
    null,
  );
  const modLangApesa = await findOrCreateModule(
  ...
  const modAgronGen = await findOrCreateModule(
    'Agronomie Générale',
    'S1',
    filiereAgronomie.id,
    null,
  );
  const modBiochim = await findOrCreateModule(
    'Biochimie Végétale',
    'S1',
    filiereAgronomie.id,
    null,
  );
  const modStatInfo = await findOrCreateModule(
    'Statistiques et Informatique Appliquées',
    'S2',
    filiereAgronomie.id,
    null,
  );
  const modPedologie = await findOrCreateModule(
    'Pédologie et Sciences du Sol',
    'S2',
    filiereAgronomie.id,
    null,
  );
  const modBotanique = await findOrCreateModule(
  ...
  await findOrCreateElement(
    'Morphologie et Taxonomie Végétale',
    modBotanique.id,
    classAgro1.id,
    ElementType.CM,
    25,
  ...
  const modFitopathol = await findOrCreateModule(
    'Fitopatologie et Protection des Plantes',
    'S5',
    filiereAgronomie.id,
    null,
  );
  const modIrrig = await findOrCreateModule(
    'Irrigation et Drainage',
    'S5',
    filiereAgronomie.id,
    null,
  );
  const modEconAgri = await findOrCreateModule(
    'Économie Agricole',
    'S6',
    filiereAgronomie.id,
    null,
  ...
  const modPhytotech = await findOrCreateModule(
    'Phytotechnie Spéciale',
    'S7',
    filiereAgronomie.id,
    optPVPP.id,
  );
  const modProtPlantes = await findOrCreateModule(
  ...
  const modAnatomie = await findOrCreateModule(
    'Anatomie Vétérinaire',
    'S1',
    filiereVeto.id,
    null,
  );
  const modHistoEmb = await findOrCreateModule(
    'Histologie et Embryologie',
    'S1',
    filiereVeto.id,
    null,
  );
  const modPhysioVet = await findOrCreateModule(
    'Physiologie Animale',
    'S2',
    filiereVeto.id,
    null,
  );
  const modZootechnie = await findOrCreateModule(
    'Zootechnie Générale',
    'S2',
    filiereVeto.id,
    null,
  ...
  const modMecaFluides = await findOrCreateModule(
    'Mécanique des Fluides',
    'S1',
    filiereGR.id,
    null,
  );
  const modTopographieGR = await findOrCreateModule(
    'Topographie et SIG',
    'S2',
    filiereGR.id,
    null,
  );
  const modHydroGR = await findOrCreateModule(
    'Hydrologie',
    'S1',
    filiereGR.id,
    null,
  ...
  // IAA 1A modules
  const modMicroIAA = await findOrCreateModule(
    'Microbiologie Alimentaire',
    'S1',
    filiereIAA.id,
    null,
  );
  const modChimIAA = await findOrCreateModule(
    'Chimie des Aliments',
    'S1',
    filiereIAA.id,
    null,
  );
  const modTechCons = await findOrCreateModule(
  ...
  await findOrCreateElement(
    'Glucides et Lipides Alimentaires',
    modChimIAA.id,
    classIAA1.id,
    ElementType.CM,
    30,
  ...
  const cMathTD1 = await upsertCours(
    'Analyse et Algèbre — Groupe TD 1',
    ElementType.TD,
  );
  const cMathTD2 = await upsertCours(
    'Analyse et Algèbre — Groupe TD 2',
    ElementType.TD,
  );
  ...
  const cAlgoProg = await upsertCours(
    'Algorithmique et Programmation — TP',
    ElementType.TP,
  );

  await assignCoursToClass(
    cMathCM.id,
    classAPESA1.id,
    teacher('NN144144'),
    null,
  ...
  // Agronomie 1A
  const cAgronCM = await upsertCours(
    "Introduction à l'Agronomie — CM",
    ElementType.CM,
  );
  const cAgronTD = await upsertCours(
    'Pratiques Agronomiques — TD',
    ElementType.TD,
  );
  const cAgronTP = await upsertCours('TP Agronomie — Groupe 1', ElementType.TP);
  const cBiochCM = await upsertCours(
    'Biochimie Structurale — CM',
    ElementType.CM,
  );
  ...
  // Vétérinaire 1A
  const cAnatCM = await upsertCours(
    'Anatomie des Carnivores — CM',
    ElementType.CM,
  );
  const cAnatTP = await upsertCours('Dissection — TP Groupe 1', ElementType.TP);
  const cHistoCM = await upsertCours(
    'Histologie des Tissus — CM',
    ElementType.CM,
  );
  const cHistoTP = await upsertCours(
    'Histologie — TP Microscope',
    ElementType.TP,
  );
  const cPhysioCM = await upsertCours(
    'Physiologie de la Digestion — CM',
    ElementType.CM,
  );
  ...
  // IAA 1A
  const cMicroCM = await upsertCours(
    'Microbiologie Générale — CM',
    ElementType.CM,
  );
  ...
  await upsertRoomReservation(
    'Amphi A',
    1,
    '08:00',
    '10:00',
    'Pr. Alaoui',
    RoomReservationPurpose.cours,
    'Biologie végétale IAG1',
  );
  await upsertRoomReservation(
    'Amphi A',
    3,
    ...
    '16:00',
    'Pr. Fennich',
    RoomReservationPurpose.examen,
    'Examen mi-parcours IGR',
  );
  await upsertRoomReservation(
    'Amphi C',
    2,
  ...
  await upsertRoomReservation(
    ...
    '11:00',
    'Direction',
    RoomReservationPurpose.reunion,
  ...
  const ensureClassStudents = async (
    entry: (typeof allDemoClasses)[number],
  ) => {
    ...
    for (let i = existing.length; i < target; i++) {
      ...
      const firstName =
        demoFirstNames[(generatedStudentIndex + i) % demoFirstNames.length];
      const lastName =
        demoLastNames[
          (generatedStudentIndex + entry.class.id + i) % demoLastNames.length
        ];
      ...
      const student: StudentDef = {
        ...
        prepaTrack: entry.cycle === StudentCycle.prepa ? 'Math-Physique' : null,
        entryLevel:
          entry.cycle === StudentCycle.prepa ? null : entry.entryLevel,
        filiereId: entry.filiereId,
        classId: entry.class.id,
        bacType:
          i % 2 === 0 ? 'Sciences Mathématiques A' : 'Sciences Physiques',
      ...
      await addHistory(
        ...
        entry.class.id,
        '2024/2025',
        entry.entryLevel ?? entry.class.year,
        'en cours',
      );
    }
  };
  ...
  const upsertCoursForElement = async (
    elementId: number,
    name: string,
    type: ElementType,
  ) => {
  ...
  const upsertClassRoomReservation = async (
    classId: number,
    roomName: string,
    slotIndex: number,
    reservedBy: string,
  ...
  for (const entry of allDemoClasses) {
    ...
    for (let i = 0; i < demoElementLabels.length; i++) {
      ...
      const element = await findOrCreateElement(
        `${entry.class.name} - ${label.element}`,
        module.id,
        entry.class.id,
        label.type,
        label.volumeHoraire,
      ...
      const cours = await upsertCoursForElement(
        element.id,
        `${entry.class.name} - ${label.element}`,
        label.type,
      );
    ...
    for (let i = 0; i < classElements.length; i++) {
      ...
      await upsertSession(
        el.name,
        entry.class.id,
        demoSlots[i].dayOfWeek,
        demoSlots[i].startTime,
        demoSlots[i].endTime,
        el.teacherId,
        demoRoomNames[(entry.class.id + i) % demoRoomNames.length],
      );
      await upsertClassRoomReservation(
        entry.class.id,
        demoRoomNames[(entry.class.id + i) % demoRoomNames.length],
        i,
        `Planning ${entry.class.name}`,
        i === 2 ? RoomReservationPurpose.examen : RoomReservationPurpose.cours,
        `${el.name} - ${entry.class.name}`,
      );
    }
      ...
      if (restaurantForDemo && mealsForDemo.length) {
        ...
        const reservation = await prisma.mealReservation.upsert({
          ...
          update: {
            ...
            status:
              si % 3 === 0
                ? MealReservationStatus.consumed
                : MealReservationStatus.confirmed,
        ...
        const existingTransaction = await prisma.mealTransaction.findFirst({
          where: {
            ...
            type: MealTransactionType.debit,
          },
          select: { id: true },
        });
    ...
    for (const session of sessions) {
      for (let si = 0; si < Math.min(students.length, 8); si++) {
        ...
        await prisma.attendanceRecord.upsert({
          ...
          update: {
            status:
              si % 5 === 0 ? AttendanceStatus.absent : AttendanceStatus.present,
            timestamp: si % 5 === 0 ? null : new Date(),
            method:
              si % 5 === 0 ? AttendanceMethod.manual : AttendanceMethod.qr,
    ...
    if (workflowStudent) {
      ...
      if (!existingTask) {
        const task = await prisma.workflowTask.create({
          data: {
            title:
              "Vérifier l'accréditation des cours de la classe Agronomie 1A",
            description:
              'Contrôler la cohérence entre plan accrédité, cours assignés et emploi du temps.',
            assignedToId: adminUser.id,
            studentId: workflowStudent.id,
            status: 'in_progress',
          },
  ...
  const [
    ...
    totalAttendance,
    totalInternatRooms,
    totalInternatAssignments,
    totalDocuments,
    totalDocumentTemplates,
    totalCoursResources,
    totalMealReservations,
  ] = await Promise.all([
  ...
  console.log(
    `   Teachers: ${teacherDefs.length} (10 permanent + 5 vacataires)`,
  );
  ...
  console.log(
    `   Internat rooms/assignments: ${totalInternatRooms}/${totalInternatAssignments}`,
  );
  console.log(
    `   Documents/resources: ${totalDocuments}/${totalCoursResources}`,
  );
