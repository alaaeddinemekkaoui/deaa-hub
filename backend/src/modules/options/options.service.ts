...
export class OptionsService {
  ...
  async findAll(query: OptionQueryDto) {
    ...
    if (search) filters.push({ OR: [{ name: { contains: search, mode: 'insensitive' } }, { code: { contains: search, mode: 'insensitive' } }] });
    if (filiereId) filters.push({ filiereId });
    if (departmentId) filters.push({ filiere: { is: { departmentId } } });

    const where: Prisma.OptionWhereInput = filters.length ? { AND: filters } : {};
  ...
  private async ensureNameAvailable(name: string, filiereId: number, excludeId?: number) {
    const ex = await this.prisma.option.findFirst({
      where: { name: { equals: name, mode: 'insensitive' }, filiereId, ...(excludeId ? { id: { not: excludeId } } : {}) },
      select: { id: true },
    });
    if (ex) throw new ConflictException(`Option "${name}" already exists in this filière`);
  }
}
